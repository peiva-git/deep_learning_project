...
def display_random_images(test_data, noisy_test_data, predicted_output, num_images=10):
    # Randomly select 10 indices from the test dataset
    random_indices = random.sample(range(len(test_data)), num_images)
    display_images(test_data, noisy_test_data, predicted_output, random_indices, num_images)


def display_images(test_data, noisy_test_data, predicted_output, image_indexes, num_images):
    # Randomly select 10 indices from the test dataset
    ...
    for i, idx in enumerate(image_indexes):
        ...
        plt.imshow(predicted_output[idx])
        plt.axis('off')
